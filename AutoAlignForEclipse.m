...
%% initial variable
clear;
threshold = 50/255;             %Sun detection threshold 
P = 1000;                       %Remove small objects fewer pixel than P
crop = 2500;                    %crop size
resize = 500;                   %resize to
openDir = './solar_eclipse';        %image folder, exmaple './solar_eclipse'
fileType = '*.JPG';             %image file type, exmaple '*.JPG', support jpg/bmp/png/tif
destination = './destination/';        %save folder './destination/'
%% open file
if ~isfolder(openDir)
...
filePattern = fullfile(openDir, fileType);
imageFiles = dir(filePattern);
%% processing
for index = 1:length(imageFiles)
    baseFileName = imageFiles(index).name;
    fullFileName = fullfile(openDir, baseFileName);
    fprintf(1, 'Now reading %s\n', fullFileName);
    A = imread(fullFileName);         
    if size(A,3) >1
        B = rgb2gray(A);         %rgb to grayscale
    else
        B = A;
    end
    ...
    boundary(:,1) = swap;
%% MEC
[center,maxR] = minboundcircle(boundary(:,1),boundary(:,2),1);
%% crop resize write image
    new = imresize(imcrop(imtranslate(A,[crop/2-center(1) crop/2-center(2)],'FillValues',0),[0 0 crop crop]),resize/crop);
    filename = strcat(destination,baseFileName);
    fprintf(1, 'Now writing %s\n', filename);
    imwrite(new,filename);
end
fprintf(1, '*******finish*******\n\n');
%end of main script

%% =================== Sub Function  minboundcircle ======================
